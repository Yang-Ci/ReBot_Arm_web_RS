...
class ArmActions:
    ...
    def execute_follow_joint_trajectory(self, goal_handle):
        ...
        try:
            ...
            point_times = self._velocity_limited_point_times(targets, point_times)
    ...
    @staticmethod
    def _velocity_limited_point_times(
        targets: list[np.ndarray], point_times: list[float]
    ) -> list[float]:
        if not point_times:
            return []
        limited = [max(float(point_times[0]), 0.0)]
        for index in range(1, len(point_times)):
            max_delta = float(np.max(np.abs(targets[index] - targets[index - 1])))
            minimum = limited[-1] + max_delta / _MAX_ACTION_JOINT_SPEED_RAD_S
            requested = max(float(point_times[index]), limited[-1])
            limited.append(max(requested, minimum))
        return limited
