...
_SAFE_HOME_GRIPPER_VLIM_RAD_S = 1.5
...
class HardwareManager:
    ...
    def __init__(
        ...
        # Low-level interpolation: smoothly move from current position to
        # the web command target over a short duration.
        ...
        self._interp_duration: float = 0.25  # seconds
        self._interp_gripper_active = False
        self._interp_gripper_start: float = 0.0
        self._interp_gripper_target: float = 0.0
        self._interp_gripper_t0: float = 0.0
    ...
    def safe_home(self) -> None:
        with self._cmd_lock:
            ...
            # 500 Hz holding loop cannot restore stale commands afterwards.
            self._interp_active = False
    ...
    def _move_gripper_for_safe_home(self) -> bool:
        """Close the gripper at a dedicated, conservative homing speed."""
        with self._cmd_lock:
        ...
        while time.monotonic() < deadline:
            last_position = self.get_gripper_state()[0]
    ...
    def _begin_gripper_command(self, *, allow_endpos: bool = False) -> None:
        ...
        if allow_endpos and not self.control_loop_active:
            # A gripper-only MIT command needs the shared 500 Hz sender.  Hold
