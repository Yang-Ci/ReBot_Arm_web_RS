...
- 固定上游资源：`rebotarm_ros2/third_party/reBot-B601-RS-for-mujoco_sim/`
- 应提交的本地覆盖：`vendor_overrides/reBot-B601-RS-for-mujoco_sim/`

不要只修改 `third_party`：重新执行 setup 后不可复现。对 SDK 的改动应制作到 `patches/`，
对模型资源的改动应同步到 `vendor_overrides/`。
...
```bash
REBOTARM_MUJOCO_VIEWER=false REBOTARM_START_AGENT=false ./rebotarm start rs_sim
```
...
source scripts/rs_env.sh
ros2 node list
ros2 topic hz /rebotarm_rs/joint_states
ros2 topic hz /rebotarm_rs/mujoco/joint_states
ros2 topic echo --once /rebotarm_rs/arm_status
```
...
1. `control_target` 是否与输入同步：不同步则检查网页阻尼、死区、60 Hz 节流和 rosbridge；
2. `control_reference` 是否连续并能追上目标：异常则检查 `vlim` 和限加速度/jerk 参数；
3. `joint_states` 是否跟随参考：异常则检查 CAN 丢帧、负载、摩擦、MIT 增益和电机状态；
4. ROS 反馈正常但网页异常：检查 80 ms rosbridge 节流、低通与显示插值；
5. 只有 TCP 异常：检查 IK 是否接近奇异点/关节限位，以及浏览器 console 中的残差；
6. 只有夹爪异常：区分开口宽度 m、电机角度 rad 和单指视觉行程 m。

夹爪拖动还应确认命令订阅 QoS 深度为 1，浏览器只按绘制帧发布最新值。若松手后仍继续
移动，通常是另一个命令源或旧进程仍在发布，而不是应该继续增大插值速度。
...
### `fastrtps_port... open_and_lock_file failed`

通常是上次驱动被 `Ctrl+Z` 暂停或异常退出。重新运行 `./rebotarm start rs` 会自动替换旧
实例并执行 `fastdds shm clean`。仍存在时先用 `ps`、`ros2 node list` 确认没有活控制器，
不要直接删除整个 `/dev/shm`。

### 滑块一松手就回零

检查网页选择的命名空间、`arm_status.enabled`、状态机是否在拒绝低层命令，以及命令话题
是否有订阅者：

```bash
ros2 topic info /rebotarm/joints/joint1/cmd/mit -v
ros2 topic echo --once /rebotarm/arm_status
```

### 网页模型先到，真机后到

真机模式必须以 `/joint_states` 驱动实线模型。目标只能显示为半透明影子；不要在发送命令
时直接把实线机器人设为目标角。也要接受速度限制会让真机实际到位时间更长。

### 真机到位后网页仍抖

对比原始 `joint_states` 与网页显示。原始值抖动需要处理机械/电机反馈；原始稳定而网页抖动
才调滤波死区。死区过大会导致小动作消失，截止频率过低会显著增加拖拽延迟。

### 安全回零后恢复旧姿态

说明某一命令源还在持续发布旧目标。检查是否有第二个网页、脚本或轨迹客户端；安全回零
本身会清控制器内部目标，但无法阻止外部节点在状态恢复后重新发送。

## 15. 版本与发布约定

- 未明确要求时，不执行 commit、push、PR 或发布；
- 不提交 `.env`、密钥、ROS bag、`build/`、`install/`、`log/` 和 third-party 嵌套仓库；
- 修改控制参数必须在仿真和低速真机上分别验证；
- README 只保留快速入口，设计原理放在对比和数据流文档；
- 发布前执行 `./setup.sh --check`、单元测试、colcon 测试和无窗口冒烟测试。
